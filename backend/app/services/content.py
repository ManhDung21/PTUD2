...
2. PHÂN TÍCH THỊ TRƯỜNG VÀ GỢI Ý GIÁ BÁN: Nếu người dùng là Plus/Pro, bạn phải thêm dấu |||PRO||| sau phần Từ khóa rồi viết MỘT ĐOẠN NGẮN GỌN (tối đa 3 dòng) gồm: mức giá gợi ý và 1-2 chiến lược bán hàng hữu ích. Dùng emoji cho sinh động.{market_text}
"""
...
Từ khóa:
[Thẻ tag tiếng Việt...]{pro_template}
...
def generate_from_image(api_key: str, image: Image.Image, style: str, product_info: Optional[str] = None, user_name: Optional[str] = None, user_tier: str = "free", anthropic_api_key: Optional[str] = None, db: Optional[Database] = None) -> str:
    ...
    tools = None # Temporarily disabled due to Google API SDK breaking changes
    model_name = "gemini-2.5-flash" # if user_tier in ["plus", "pro"] else "gemini-2.5-flash"
